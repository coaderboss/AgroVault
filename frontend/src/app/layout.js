    ...
    <html lang="en">
      <body className="bg-gray-50 font-sans text-gray-900 antialiased overflow-hidden">
        <div className="flex h-screen w-full">
          ...
          {/* ─── MAIN CONTENT ─── */}
          <div className="flex-1 flex flex-col h-screen w-full bg-gray-50/50">
            ...
            {/* MOBILE BOTTOM NAV */}
            <nav className="md:hidden fixed bottom-0 w-full bg-white/95 backdrop-blur-xl border-t border-gray-200 shadow-lg z-50">
              <div className="flex justify-around items-center h-16 px-2">
                {navItems.slice(0,4).map((item) => {
                  const Icon = item.icon;
                  const isActive = pathname === item.path;
                  return (
                    <Link key={item.name} href={item.path} className="flex-1 flex flex-col items-center justify-center gap-1">
